...
namespace litehtml
{
	...
	bool litehtml::html_tag::set_class(const char* pclass, bool add)
	{
		...
		if(changed)
		{
			...
			// Also mark all descendants dirty — CSS descendant selectors
			// like ".selected td" require children to re-evaluate styles
			// when this element's class changes.
			std::function<void(element*)> mark_children_dirty = [&](element* el) {
				for(auto& child : el->children())
				{
					child->m_style_dirty = true;
					mark_children_dirty(child.get());
				}
			};
			mark_children_dirty(this);
