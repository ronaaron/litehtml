...
{
	// Skip if parent width hasn't changed — the 12 calc_percent results are deterministic
	// But invalidate if styles changed (e.g. from set_class), since CSS values may differ
	if(m_element->is_style_dirty())
		m_last_outlines_parent_width = -1;
	if(parent_width == m_last_outlines_parent_width)
		return;
	m_last_outlines_parent_width = parent_width;
