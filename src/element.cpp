...
#define LITEHTML_RETURN_FUNC(ret)                                                                                      \
	...
	bool element::find_styles_changes(position::vector& redraw_boxes)
	{
		...
		// Skip entire subtrees that have no dirty elements.
		// m_style_dirty is set on an element and propagated to ancestors
		// when set_pseudo_class() or set_class() changes state.
		if(!m_style_dirty)
		{
			return false;
		}
